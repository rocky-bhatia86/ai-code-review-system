...
class GitHubService:
    ...
    def get_pr_diff(self, repo_owner: str, repo_name: str, pr_number: int) -> Optional[str]:
        ...
        try:
            ...
            print(f"🔍 DEBUG: Requesting PR diff from: {url}")
            print(f"🔍 DEBUG: Headers: {headers}")
            
            response = requests.get(url, headers=headers)
            print(f"🔍 DEBUG: PR diff response status: {response.status_code}")
            print(f"🔍 DEBUG: PR diff response length: {len(response.text)}")
            
            response.raise_for_status()
            
            if not response.text.strip():
                print("⚠️  DEBUG: PR diff response is empty or whitespace only")
            else:
                print(f"✅ DEBUG: PR diff response has content, first 100 chars: {repr(response.text[:100])}")
            
            return response.text
            
        except requests.RequestException as e:
            print(f"❌ DEBUG: Error fetching PR diff: {e}")
