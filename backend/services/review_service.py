...
class ReviewService:
    ...
    def parse_diff_for_inline_comments(self, diff_content: str, line_comments: list) -> list:
        ...
        current_file = None
        line_mapping = {}  # Maps diff line number to (file_path, new_line_number)
        new_line_number = 0
        ...
        for diff_line_num, line in enumerate(diff_lines, 1):
            if line.startswith('diff --git'):
                ...
                if match:
                    new_line_number = int(match.group(1)) - 1  # Start from line before
                    print(f"📍 DEBUG: Hunk starts at line {new_line_number + 1}")
            elif line.startswith('+') and not line.startswith('+++'):
                # This is an added line
                new_line_number += 1
                if current_file:
                    line_mapping[diff_line_num] = (current_file, new_line_number)
            elif not line.startswith('-') and not line.startswith('\\'):
                # Context line (unchanged)
                new_line_number += 1
        
        print(f"🗺️  DEBUG: Created {len(line_mapping)} line mappings")
        ...
        for i, comment in enumerate(line_comments):
            ...
            if ai_line in line_mapping:
                file_path, actual_line = line_mapping[ai_line]
                mapped_comments.append({
                    **comment,
                    'file_path': file_path,
                    'actual_line': actual_line
                })
                print(f"✅ DEBUG: Exact match - {file_path}:{actual_line}")
            else:
                # Fallback: find the closest added line within range
                best_match = None
                best_distance = float('inf')
                
                for diff_line, (file_path, actual_line) in line_mapping.items():
                    distance = abs(diff_line - ai_line)
                    if distance <= 5 and distance < best_distance:  # Within 5 lines
                        best_match = (file_path, actual_line)
                        best_distance = distance
                
                if best_match:
                    file_path, actual_line = best_match
                    mapped_comments.append({
                        **comment,
                        'file_path': file_path,
                        'actual_line': actual_line
                    })
                    print(f"🎯 DEBUG: Fuzzy match - {file_path}:{actual_line} (distance: {best_distance})")
                else:
